...
class Database {
  ...
  #validateFolderPath(folderPath) {
    try {
      if (!FS.statSync(folderPath).isDirectory()) throw new Error('Invalid path for collection. Provided path should lead to a folder.');
    } catch (e) {
  ...
  #validatePath(dataFile) {
    if (path.extname(dataFile) !== '.json') throw new Error('Invalid file path for database. Provided path should lead to a .json file.');
