...
class Database {
  ...
  get(key, decrypt=false) {
    if (![true, false].includes(decrypt)) throw new TypeError('Parameter \'decrypt\' must be of type boolean.');
   ...
   * @param {boolean} [encrypt=false] Whether or not to encrypt the value and save it after. Defaults to false
   * @returns {any}
   */
  set(key, value, encrypt=false) {
    if (![true, false].includes(encrypt)) throw new TypeError('Parameter \'encrypt\' must be of type boolean.');

    if (!this.config.encryptionKey || encrypt === false) set(this.data, key, value);
    else if (encrypt === true) set(this.data, key, this._encrypt(value));
  ...
  _validateBeforeDecrypt(value) {
    if (Object.prototype.toString.call(value) !== "[object String]") throw new TypeError('Provided value has to be a string to be decrypted.');
  }

  _validateBeforeEncrypt(value) {
    if (Object.prototype.toString.call(value) !== "[object String]") throw new TypeError('Provided value has to be a string to be encrypted.');
    else if (value.split(':').length !== 2 || value.includes(' ')) throw new TypeError('Provided value cannot be decrypted as it was not encrypted before.');
  }
