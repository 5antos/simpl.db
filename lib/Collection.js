...
class Collection {
  ...
  createBulk(...entries) {
    ...
    this.#data.push(...entries.map(entry => ({ ...entry, ...(this.#config.timestamps ? { createdAt: now, updatedAt: now } : {}) })));

    if (this.#config.autoSave) this.save();

    return entries.map(entry => ({ ...Object.fromEntries(Object.entries(this.#defaultValues).filter(([k]) => !Object.keys(this.#defaultValues).includes(k))), ...entry }));
  }
    ...
    for (var i = 0; i < defaults.length && i <= bulkIndex; i++)
      if (!entry.hasOwnProperty(defaults[i][0]))
        if (defaults[i][0].startsWith('$')) {
          ...
          entry[propertyName] = isNaN(value) ? defaults[i][1] : value + 1 + bulkIndex;
        }
        else
