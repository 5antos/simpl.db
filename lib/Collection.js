 ...
 * @typedef {import('simpl.db').Filter} Filter
 */
...
class Collection {
   ...
   * @param {Filter} [filter] Filter to apply. If this parameter is not provided, all data from the collection will be returned
   * @returns {Data|Data[]}
   */
  get(filter=(() => true)) {
   ...
   * @param {Filter} [filter] Filter to apply. If this parameter is not provided, all data from the collection will be removed
   * @returns {Data[]}
   */
  remove(filter=(() => true)) {
    if (typeof filter !== 'function') throw new TypeError('The filter must be a function.');
   ...
   * @param {Filter} [filter] Filter to apply. If this parameter is not provided, all data from the collection will be updated
   * @returns
   */
  update(data, filter) {
    if (!data || (typeof data !== 'function' && !isObject(data))) throw new Error('The data must be an object.');
