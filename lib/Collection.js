...
class Collection {
  ...
  #checkEntry(entry, bulkIndex=0) {
    ...
    const entryKeys = Object.entries(entry).filter(([k]) => k.startsWith('_'));

    for (var j = 0; j < entryKeys.length; j++) {
      const key = entryKeys[j][0].slice(1);
      const queryObject = entryKeys[j][1];
  
      delete entry[entryKeys[j][0]];
  
      if (!isObject(queryObject)) throw new TypeError(`The value for the key _${key} must be an object`);
  
      const filterKeys = Object.entries(queryObject).filter(([k]) => k !== 'collection');
  
      if (!isValidKey(queryObject.collection) || !this.database.getCollection(queryObject.collection)) return;
      else if (!filterKeys.length) return;
  
      const val = this.database
        .getCollection(queryObject.collection)
        .get((target) => filterKeys.every(([k, v]) => target[k] === v));
